...
async def id():
    return await id_obj.get_sequence_id()


async def redis_id(num=1):
    day = int(lib.common.func.get_ts() / 86400)
    num = await redis_client.incr_amount(SequenceRedis.SEQUENCE_ID.format(day), num)
...
class IdSequence:
    ...
    async def get_sequence_id(self):
        if self.sequence_id_list:
            return self.sequence_id_list.pop(0)
        else:
            self.sequence_id_list = [i for i in range(await redis_id(1), await redis_id(ID_QUEUE))]
